...
export default class EditTeacherAssingnment extends Component {
  constructor() {
    super();
    this.state = {
      loggedInUser: {},
  ...
  async componentWillMount() {
    ...
    var formatedDate = Moment(this.props.submition_date).format('MM/DD/YYYY');
    var param = {       
        id: this.props.id,
        attachment: this.props.attachment,
        class_id: this.props.class_id,
        text: this.props.description,
        member_no: this.props.member_no,
  ...
  chooseAssignment()
  {
    DocumentPicker.show({
      ...
      this.setState({attachment:res.fileName})
  ...
  onSubmitSearch(){
    ...
    var objThis = this;
    /* Call function in teacher services to upload assignment only text */
    ...
    var formatedDate = Moment(this.state.fromDate).format('YYYY-MM-DD');
    var obj = this;
    var url = config.api_url + ':' + config.port + '/assignment/update?token=' + config.api_token;
    var configHeader = {
      headers: {
            ...
            <ScrollView>
              ...
              <View style={styles.dashcontainer} >
                <View style={[styles.whitebackgrounnd, styles.padding]} >
                  <View>
                    <TextInput style={styles.TextInputStyleClass} placeholder="Title" onChangeText={(value) => this.setState({titles:value})} value={this.state.titles} />
                    <DatePicker
                  ...
                  <View>
                    <Text>{this.state.attachment}</Text>                 
                  </View>
